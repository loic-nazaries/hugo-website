...
This repository is the **single source of truth** for my personal website — both the Hugo source files and the generated `public/` output live here.
...
The `public/` folder is tracked in this repository and kept in sync automatically:

- A **pre-push git hook** (`.githooks/pre-push`) runs `hugo` before every push, rebuilds `public/`, and commits any changes automatically.
- **GitHub Actions** then deploys the updated `public/` folder to GitHub Pages on every push to the main branch.
